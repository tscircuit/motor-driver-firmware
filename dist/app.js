...
function controls(){const live=!!port&&lastSeen>0&&Date.now()-lastSeen<2500;$('beep').disabled=!live;$('threshold').disabled=!live;$('apply').disabled=!live||!!last?.motor_enabled;for(const id of ['mode','resolution','direction','steps','speed','start'])$(id).disabled=!live||last?.protocol!==3||!!last?.motor_enabled;$('stop').disabled=!port;$('connect').disabled=connecting;$('connect').textContent=port?'Disconnect':'Connect board ↗';}
...
function receive(line){
 ...
 last=data;lastSeen=now;points.push({t:now,v:t});points=points.filter(p=>p.t>=now-300000).slice(-1300);
...
async function connect(){if(port)return disconnect();connecting=true;controls();try{
 ...
 await selected.open({baudRate:115200});port=selected;lastSeen=0;last=null;points=[];dirty=false;
...
else navigator.serial.addEventListener('disconnect',e=>{if(e.target===port||e.port===port){message('Board unplugged. Reconnect to continue.');void disconnect();}});
...
function heartbeat(){if(!port?.writable||document.hidden)return;const target=port;writeChain=writeChain.catch(()=>{}).then(async()=>{if(!target.writable)return;const w=target.writable.getWriter();try{await w.write(new TextEncoder().encode('{"cmd":"heartbeat"}\n'));}finally{w.releaseLock();}}).catch(()=>{});}

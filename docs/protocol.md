...
Telemetry runs every 125 ms (8 readings per second) and retains existing protocol-3 fields (temperature/alarm, fault state, motion counts/state, current availability, USB naming, device ID, LED state). New identity fields are `board_id`, `mcu`, and `driver`.
...
- `heartbeat_timeout_ms`, `buzzer_hz`, `telemetry_interval_ms`.
